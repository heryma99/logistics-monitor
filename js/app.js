...
var App = (function(){
  ...
  function transit(){
    ...
    var hist = ((D.transit&&D.transit.history)||[]).map(h=>'<tr><td>'+esc(h.lane)+'</td><td>'+h.n+'</td><td>'+h.avg_actual+'</td><td style="color:var(--tx2)">'+h.avg_ref+'</td><td style="color:'+(h.gap>0?"var(--red)":"var(--green)")+'">'+(h.gap>0?"+":"")+h.gap+'</td><td style="color:'+(h.over_pct>30?"var(--amber)":"var(--tx2)")+'">'+h.over_pct+'%</td></tr>').join("");
    var histCard = '<div class="card"><h3>历史段偏差（已签收货件：ATD→ATA 实际 vs 参考物流+查验）</h3><table><tr><th>线路（国家·渠道）</th><th>票数</th><th>实际均(天)</th><th>参考均(天)</th><th>偏差</th><th>超参考占比</th></tr>'+hist+"</table><p class=\"muted\" style=\"margin-top:6px\">口径注：实际=ATD→ATA(到港/到仓)；参考=物流+查验至派送，故空运类实际常小于参考属正常，正偏差(+天)才是真延误。超 180 天的异常记录已隔离（见下方）。</p></div>"
      + (function(){
          var sg = ((D.transit&&D.transit.segments)||[]).slice(0,10).map(r=>'<tr><td>'+esc(r.lane)+'</td><td>'+r.n+'</td><td>'+(r['seg1_提货到离港']!=null?r['seg1_提货到离港']:'—')+'</td><td>'+(r['seg2_头程到放行']!=null?r['seg2_头程到放行']:'—')+'</td><td>'+(r['seg3_放行到到仓']!=null?r['seg3_放行到到仓']:'—')+'</td><td>'+(r['seg4_到仓到派送']!=null?r['seg4_到仓到派送']:'—')+'</td><td style="color:var(--amber)">'+esc(r.bottleneck_seg||'')+'</td></tr>').join("");
          var an = ((D.transit&&D.transit.anomalies)||[]).map(a=>'<tr><td>'+esc(a.fba)+'</td><td>'+esc(a.lane)+'</td><td>'+esc(a.atd)+' → '+esc(a.dlv)+'</td><td style="color:var(--red)">'+a.total_days+' 天</td><td class="muted">'+esc(a.reason)+'</td></tr>').join("");
